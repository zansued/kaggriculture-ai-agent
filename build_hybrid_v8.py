"""Build do bundle single-file do HÍBRIDO V8: Moon V56 + overlays do mix +
glut-guard com dump_floor POR ITEM + front-run MAIS SENSÍVEL.

V8 = hybrid_v6 com ÚNICA mudança: _OPP_THRESH reduzido nas frutas de colapso:
  - STRAWBERRY 4 -> 2
  - MELON 2 -> 1
  - MILK 4 / WOOL 3: MANTIDOS (pico tardio, vender cedo demais é arriscado)
  - _OPP_MAX_DAY, glut-guard, sell_first: idênticos ao v6.

Justificativa: v5->v6 validou que reduzir STRAWB 6->4 e MELON 4->2 deu 33-12.
Hipótese: frutas de colapso (STRAWB crash pós-d21, MELON pós-d10) disparam
front-run com MENOS produção do oponente, capturando preço ainda melhor antes
do glut. Isola APENAS o efeito do threshold das frutas.

Output: submissions/hybrid_v8/main.py
Usage:   python build_hybrid_v8.py
"""
...
OUT = ROOT / "submissions" / "hybrid_v8" / "main.py"
...
# ---------------------------------------------------------------------------
_OPP_THRESH = {"STRAWBERRY": 2, "MELON": 1, "MILK": 4, "WOOL": 3}
...
def build() -> None:
    ...
    header = f'''"""hybrid_v8 - single-file Kaggle submission bundle.
...
janela 250-650. Front-run MAIS SENSÍVEL: _OPP_THRESH STRAWB 2 / MELON 1
(MILK/WOOL mantidos).

Built by build_hybrid_v8.py. Self-contained: embute moon_agent_main.py

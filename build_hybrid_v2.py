"""Build do bundle single-file do HÍBRIDO V2: Moon V56 + overlays do mix + glut-guard.

V2 = hybrid_single (Moon + mature_opp_front_run + sell_first) + overlay novo
_glut_guard: timing de venda adaptativo à saturação do mercado.

Validado (engine 1.32.7, seeds 1-36):
  - hybrid_v2 vs hybrid_single: 23-12 (+133/j)  <- ganho real (p~0.03)
  - hybrid_v2 vs Moon puro:     12-0 (+1751/j)
  - hybrid_v2 vs c27:           12-0 (+22842/j)
  - hybrid_v2 vs purearch:      12-0 (+22419/j)
...
sell_first) MAIS o _glut_guard.

Output: submissions/hybrid_v2/main.py
Usage:   python build_hybrid_v2.py
"""
...
OUT = ROOT / "submissions" / "hybrid_v2" / "main.py"

# Overlays: mesmos do hybrid_single (mix_agent) + glut_guard.
_MIX_OVERPRELUDE = '''\
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# V2: _glut_guard — timing de venda adaptativo à saturação do mercado.
# Segura itens que SOBEM no final (wheat/carrot/tomato: 29->47, 35->279,
# 60->409) e antecipa o dump de itens que COLAPSAM (milk/wool/melon/strawberry:
# 175->7, 212->5). Validado 23-12 vs hybrid_single (seeds 1-36).
# ---------------------------------------------------------------------------
...
def build() -> None:
    ...
    header = f'''"""hybrid_v2 - single-file Kaggle submission bundle.

Moon V56 (public 2736-2913) + overlays do mix (maturity front-run + order-slot
sell-first) + glut-guard (timing adaptativo à saturação). Validado: 23-12 vs
hybrid_single, 12-0 vs Moon/c27/purearch (seeds 1-36, engine 1.32.7).

Built by build_hybrid_v2.py. Self-contained: embute moon_agent_main.py

"""Build do bundle single-file do HÍBRIDO V15: Moon V56 + overlays do mix +
glut-guard v6 (ótimo local, inalterado) + COMPRA DE WHEAT CEDO.

V15 = hybrid_v6 + _buy_wheat_early:
  - Todos os top agents (Crop Dusta, Ryo, tetsuya, Blu3s) COMPRAM WHEAT no
    mercado nos dias 0-13 (PROD:WHEAT, 5-20/dia) a preço baixo (~29-32) para
    FEED dos animais + reserva/arbitrage. O v6 (Moon) não compra NADA.
  - V15: nos primeiros 12 dias, se WHEAT estiver barato (< 32) e money > 800 e
    reserva de WHEAT no shed < 15, injeta uma ordem BUY_PRODUCT WHEAT qtd=5
    (se houver slot no order-slot de 10). Não altera a lógica de venda.
...
zlib+base85, e injeta inline os overlays.

Output: submissions/hybrid_v15/main.py
Usage:   python build_hybrid_v15.py
"""
...
OUT = ROOT / "submissions" / "hybrid_v15" / "main.py"
...
# ---------------------------------------------------------------------------
# V5 glut-guard (ótimo local do v6) — INALTERADO.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# V15: _buy_wheat_early — compra WHEAT barato nos primeiros 12 dias.
# Top agents compram PROD:WHEAT (5-20/dia) d0-d13 para FEED + reserva.
# ---------------------------------------------------------------------------
...
def build() -> None:
    ...
    header = f'''"""hybrid_v15 - single-file Kaggle submission bundle.

Moon V56 + overlays do mix (maturity front-run + order-slot sell-first) +
glut-guard v6 (inalterado) + _buy_wheat_early (compra WHEAT barato d0-d12
para FEED/reserva, como os top agents fazem).

Built by build_hybrid_v15.py. Self-contained: embute moon_agent_main.py

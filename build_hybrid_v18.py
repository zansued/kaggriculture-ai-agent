...
OUT = ROOT / "submissions" / "hybrid_v18" / "main.py"
...
_GSTART = 200
_GSTOP = 680
_WINDOW = 24          # janela de histórico (steps) ~ 1 dia
_MAX_LOTE = 16        # venda fracionada (padrão tops: lotes 3-16)
_MOM_THRESH = 0.04    # momentum mínimo para reagir
_FLOOR_SHIFT = 0.12   # quanto o floor muda com momentum

_PHIST = {}           # seat -> {item -> [preços]}


def _floor_of(item):
    return _GD_FLOOR.get(item, 0.45)
...
def _glut_guard(action, obs, step):
    ...
    for item in _GDUMP:
        ...
        if p > 0:
            ...
            if len(hist[item]) > _WINDOW:
                hist[item] = hist[item][-_WINDOW:]
    ...
    for o in market:
        if isinstance(o, list) and len(o) >= 3 and o[0] == "SELL" and o[1] in _GBASE:
            ...
            if item in _GDUMP:
                h = hist.get(item, [])
                if len(h) < 5:
                    # histórico insuficiente: usa base fixa (v17)
                    floor = _floor_of(item)
                else:
                    ...
                    floor = _floor_of(item)
                    if momentum < -_MOM_THRESH:
                        floor = max(0.12, floor - _FLOOR_SHIFT)   # vender antes do crash
                    elif momentum > _MOM_THRESH:
                        floor = min(0.75, floor + _FLOOR_SHIFT)   # segurar p/ pico
                    base = dyn_base  # base dinâmica
                if price >= base * floor:
                    avail = int(shed.get(item, 0) or 0)
                    if avail > 0:
                        # vende FRACIONADO (lote pequeno) — não crasha o mercado
                        lote = min(max(qty, avail), _MAX_LOTE)
...
def build() -> None:
    ...
    header = f'''"""hybrid_v18 - single-file Kaggle submission bundle.
...
Built by build_hybrid_v18.py. Self-contained.
"""
from __future__ import annotations

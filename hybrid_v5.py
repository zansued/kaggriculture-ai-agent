"""hybrid_v5: dump_floor PER-ITEM calibrado ao timing de crash de cada produto.

Mudanças vs hybrid_v4 (validado em h2h 36 seeds vs v4: 27W-8L, +7800):
  - dump_floor agora é POR ITEM (não global 0.40):
      MILK 0.45, WOOL 0.45, MELON 0.40, STRAWBERRY 0.40
    Justificativa: cada colapsante tem crash em momento diferente — MILK pico
    d13, WOOL dip+recupera, MELON crash pós-d10, STRAWBERRY pós-d21.
  - Mantém janela 250-650 (validada no v4).

Base: submissions/hybrid_single/main.py (Moon V56 + mature_opp_front_run +
sell_first). Mesmo overlay do v4, com dump_floor per-item.

Uso: python h2h_bench.py hybrid_v5.py hybrid_v4.py --seeds 1-36
     python h2h_bench.py hybrid_v4.py hybrid_v5.py --seeds 1-36   # espelho
"""
...
_DUMP_FLOOR = {"MILK": 0.45, "WOOL": 0.45, "MELON": 0.40, "STRAWBERRY": 0.40}
_GUARD_START = 250
_GUARD_STOP = 650

_ENABLED = os.environ.get("HYBRID_V5_GUARD", "1") == "1"
...
def _glut_guard(action, obs, step):
    ...
    for o in market:
        if isinstance(o, list) and len(o) >= 3 and o[0] == "SELL" and o[1] in _BASE_PRICE:
            ...
            elif item in _EARLY_DUMP and price >= base * _DUMP_FLOOR.get(item, 0.40):
